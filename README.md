...
- 免费试用 **10 篇**，无需任何注册即可使用全部功能
- 累计同步超过 10 篇后，在设置页填写**授权码**即可继续同步
...
- ⏱ **专辑增量同步单次上限**：单次同步的篇数跟随「每批同步数量」设置（5–10），积压多时从最旧一篇开始、分周期自动追平，不再一次追平全部积压；配合 AI 识别时单次同步耗时可控
- 💬 提示语显示追平进度（「新增 N 条，剩余 M 条下个周期继续」）

### v1.2.1 / v1.2.2

- 免费试用额度调整为 10 篇；修复 Obsidian 1.13 设置独立窗口下登录导致闪退的问题
